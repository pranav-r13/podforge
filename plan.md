...
## Handoff (2026-09-20, later same day, +2 features)

Shipped this session (uncommitted): two feature requests from the user.
1. **Custom output folder name** — `convert/ffmpeg.rs`'s `ConvertOptions` gained `output_folder_name: Option<String>`; `build_output_path` uses it (sanitized, same as artist/album) as a flat `{output_dir}/{name}/` subfolder in place of the default `{output_dir}/{Artist}/{Album}/` nesting when set and non-blank. `enqueue_conversion` command takes the new param and threads it through. Frontend: new "Output folder name (optional)" text input in the Convert dialog, reset to blank each time the dialog opens.
2. **Clear queue** — new `db::queries::clear_finished_jobs` (deletes `jobs` rows with status done/error/cancelled; queued/running untouched) + `commands::clear_finished_jobs` (deletes then returns the remaining list). Frontend: "Clear" button in the Jobs drawer header, shown only when finished jobs exist.
`cargo build`, `cargo test` (38 passing, added 3 new `build_output_path` tests for the custom-folder-name path), `cargo clippy --all-targets` (0 warnings, added `#[allow(too_many_arguments)]` to `enqueue_conversion` — Tauri command signature, not a real smell), `svelte-check` (0 errors) all pass. Not yet manually click-tested (same blocker as everything else below — needs a session at the keyboard).
